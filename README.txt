...
Stage 2:
[DONE]	SET ENVIRONMENT VARS, INSTALL THE SSL CERTIFICATE ON PHP FOLDER
[DONE]	Using the Twilio API, send out a message
[DONE]	Likely should store the Twilio credentials somewhere, not in the code … a database table, or a file, or a build property
[DONE]	When the user clicks the button to send the form, it should store in the database table AND send a text message to Twilio
[DONE]	The response back from Twilio should be stored in the database table

Stage 3:

[DONE]	The response back from Twilio should be stored in the database table AND should be shown in the UI
